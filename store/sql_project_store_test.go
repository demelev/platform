...
func TestProjectStoreSave(t *testing.T) {
	Setup()
	...
	o1.Type = model.PROJECT_OPEN

	if err := (<-store.Project().Save(&o1)).Err; err != nil {
...
func TestProjectStoreUpdate(t *testing.T) {
	...
	if err := (<-store.Project().Update(&o1)).Err; err != nil {
		t.Fatal(err)
	}
 ...
 *    Must(store.Project().Save(&o1))
 *
 *    if r1 := <-store.Project().Get(o1.Id); r1.Err != nil {
 *        t.Fatal(r1.Err)
 *    } else {
 *        if r1.Data.(*model.Project).ToJson() != o1.ToJson() {
 *            t.Fatal("invalid returned channel")
 *        }
 *    }
 *
 *    if err := (<-store.Project().Get("")).Err; err == nil {
